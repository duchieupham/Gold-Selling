 ...
 * @author phamduchieu
 */
public class ExecuteUpdateProductAction {
    ...
    public ExecuteUpdateProductAction() {
    }

    public String execute() throws Exception {
        String url = FAIL;
        String[] arr = thumbnailValue.split(",");
        thumbnailValue = arr[0];
        ProductDAO dao = new ProductDAO();
        ProductDTO dto = new ProductDTO();
        dto.setId(Integer.parseInt(productID));
        dto.setProductName(productName);
        dto.setPrice(Float.parseFloat(productPrice));
        dto.setQuantity(Integer.parseInt(productQuantity));
        boolean check = dao.updateProduct(dto, Integer.parseInt(catValue), Integer.parseInt(brandValue), Integer.parseInt(thumbnailValue));
        if(check){
            url = SUCCESS;
        }
        return url;
